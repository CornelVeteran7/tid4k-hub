  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-display font-bold">Documente</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Dialog open={uploadOpen} onOpenChange={setUploadOpen}>
            <DialogTrigger asChild>
              <Button className="gap-2"><Upload className="h-4 w-4" /> Încarcă</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
      ...
      {loading ? (
        ...
        <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4' : 'space-y-3'}>
          {documents.map((doc) => (
            <Card key={doc.id_info} className="overflow-hidden hover:shadow-md transition-shadow">
              <CardContent className={viewMode === 'grid' ? 'p-4' : 'p-4 flex items-center gap-4'}>
                {/* Thumbnail */}
                <div className={`${viewMode === 'grid' ? 'mb-3 h-32' : 'h-12 w-12 shrink-0'} rounded-lg bg-muted flex items-center justify-center`}>
                  {doc.tip_fisier === 'pdf' ? (
